...
class Folder extends HTMLElement {
  ...
  async refresh() {
    ...
    try {
      ...
      let pathCandidate = Object.keys(fileToRemove);

      if (pathCandidate.length) {
        let response = await Sse.fetch("send.es", {
          action: "of-path",
          paths: pathCandidate.map(inode => ({inode}))
        });
...
export const sideNav = create("nav", {class: "side-nav"}, sideNav => {
  ...
  sideNav.renameFile = function () {
    var target = sideNav.querySelector("a-file.select, a-folder.select");

    if (target) renameFile(target);
  };
  ...
  const contextMenu = create("div", {class: "context-menu"}, menu => {
    ...
    item.addEventListener("click", event => {
      renameFile(contextMenu.targetElement);
      contextMenu.dismiss();
    });
