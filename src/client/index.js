...
const mainContainer = create("div", {class: "main-container"}, mainContainer => {
  const resizer = create("a-resizerx", {}, resizer => {
        ...
        editorContainer = create("div", {class: "editor-container"}, editorContainer => {
          editorContainer.addEventListener("pointerdown", event => {
            const tartget = event.target;

            if (!tartget.closest(".cm-content") && !event.target.matches(".cm-gutterElement > [title='Fold line']")) {
        ...
        iframePanel = create("div", {class: "iframe-panel"}, iframePanel => {
          ...
          function lockedFrameByAddress(address) {
          ...
          lockBtn.addEventListener("click", () => {
            ...
            if (document.body.classList.contains("preview-off")) {
              mainContainer.lockBtn.classList.toggle("locked");
              lockedFrameByAddress(address);
              return;
            }
            if (lockBtn.classList.contains("locked")) {
...
window.addEventListener("keydown", event => {
  ...
  if (event.ctrlKey || event.metaKey) {
    ...
    if (key === "f2") {
      ...
      sideNav.querySelector(":focus")?.blur();
    } else if (key === "f1") {
      event.preventDefault();
      sideNav.copyPath();
    }
